...
# Training series: all but last 14 days (so forecast aligns with LSTM test period)
n_train <- nrow(data) - h - 1
...
```{r}
library(forecast)
diff_data <- diff(ts_data_train)

fit_1 <- Arima(ts_data_train, order = c(1,1,1))
fit_2 <- Arima(ts_data_train, order = c(2,1,1))
...
forecast_dates <- data$pickup_date[(n_train + 1):(n_train + h)]
...
# Save for comparison with LSTM (compare_forecasts.py)
arima_forecast_df <- data.frame(
  date = forecast_dates,
  actual = data$avg_duration_min[(n_train + 1):(n_train + h)],
...
```{r}
lstm_path <- file.path(getwd(), "../data", "lstm_forecast_14day.csv")
comparison <- data.frame(
  Model = "ARIMA",
  sMAPE_pct = round(arima_smape, 4),
  MASE = round(arima_mase, 4)
)
if (file.exists(lstm_path)) {
  lstm_df <- read.csv(lstm_path)
  lstm_df$actual <- as.numeric(lstm_df$actual)
  lstm_df$forecast <- as.numeric(lstm_df$forecast)
  lstm_smape <- Metrics::smape(lstm_df$actual, lstm_df$forecast) * 100
  lstm_mase <- yardstick::mase_vec(
    truth = lstm_df$actual,
    estimate = lstm_df$forecast,
    m = 1,
    mae_train = mae_train
  )
