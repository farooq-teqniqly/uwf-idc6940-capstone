...
Implementation spans four components in \texttt{code/}:

\paragraph{Preprocessing (\texttt{capstone\_project\_preprocessing.ipynb}).} A PySpark Jupyter notebook loads the NYC TLC High-Volume FHV parquet data, filters to Uber trips (\texttt{hvfhs\_license\_num == HV0003}), derives \texttt{trip\_duration\_min} and \texttt{avg\_speed\_mph}, applies basic range filters and IQR-based outlier removal, aggregates by \texttt{pickup\_date} to daily \texttt{avg\_duration\_min}, and writes the 366-day time series to CSV (see Section~\ref{sec:preprocessing} and Appendix~\ref{appendix:pyspark}).

\paragraph{ARIMA (\texttt{time\_series\_forecast.qmd}).} The R/Quarto pipeline loads \texttt{finalproject.csv} and uses the same chronological split as LSTM: last 14 days as test, preceding 30 as validation, remainder as training (\texttt{n\_train = 322}). It runs stationarity tests (ADF), ACF/PACF plots, and model comparison (ARIMA(1,1,1), ARIMA(2,1,1), and \texttt{auto.arima}, which selects ARIMA(3,1,2)). A 44-step forecast (\texttt{forecast(..., h = VAL\_DAYS + h}) is produced; the last 14 steps are extracted to align with the LSTM test window. The pipeline outputs a formatted forecast table (point forecast, Lo/Hi 80\%, Lo/Hi 95\%), computes sMAPE and MASE using \texttt{Metrics::smape} and \texttt{yardstick::mase\_vec}, and writes \texttt{arima\_forecast\_14day.csv} (columns: \texttt{date}, \texttt{actual}, \texttt{forecast}) to \texttt{../data/} for comparison. It also includes an optional side-by-side comparison with LSTM when \texttt{lstm\_forecast\_14day.csv} is present.
