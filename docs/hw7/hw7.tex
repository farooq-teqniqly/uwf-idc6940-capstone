...
\begin{table}[ht]
\centering
\caption{Comparison of ARIMA and LSTM on 14-day forecast (sMAPE and MASE).}
\label{tab:compare}
\begin{tabular}{lcc}
\toprule
Model & sMAPE (\%) & MASE \\
...
\begin{enumerate}
  \item \textbf{Stabilize LSTM reporting.} When reporting final metrics, either fix the random seed in the LSTM pipeline and re-run once, or run multiple times and report mean and standard deviation of sMAPE/MASE so the comparison with ARIMA is interpretable.
...
The dataset, QMD files, PySpark notebook, and outputs are also available for download via a public GitHub release at \url{https://github.com/farooq-teqniqly/uwf-idc6940-capstone-files/releases/tag/capstone-v0.1}
...
The following R code (from \texttt{code/arima\_forecast\_final.qmd}) produces the full-year time series plot in Figure~\ref{fig:fullyear_ts}.
...
The following R code produces the 14-day comparison plot (Figure~\ref{fig:comparison_plot}). It assumes \texttt{plot\_df} contains columns \texttt{date}, \texttt{actual}, \texttt{arima}, and \texttt{lstm} (e.g., from merging the ARIMA and LSTM forecast CSVs).
...
The following R code (from \texttt{code/arima\_forecast\_final.qmd}) produces the ARIMA 14-day forecast plot with 80\% and 95\% prediction intervals. It assumes \texttt{data}, \texttt{n\_train}, \texttt{h}, \texttt{forecast\_dates}, and \texttt{forecast\_summary} (with columns \texttt{Point Forecast}, \texttt{Lo 80}, \texttt{Hi 80}, \texttt{Lo 95}, \texttt{Hi 95}) are already defined from the ARIMA pipeline.
