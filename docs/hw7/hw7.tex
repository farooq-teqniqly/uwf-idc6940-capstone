...
Figure~\ref{fig:arima_intervals} shows the ARIMA 14-day point forecast together with 80\% and 95\% prediction intervals. The plot is produced by the code in Listing~\ref{lst:arima_intervals_plot}. \textbf{How to interpret the graph:} The gray line is the historical daily average trip duration (training data) up to the start of the forecast period. The dark blue line and points are the ARIMA point forecast for the next 14 days. The two shaded blue bands represent uncertainty: the wider, lighter band is the 95\% prediction interval (we expect the actual value to lie in this band with probability 0.95), and the narrower, darker band is the 80\% interval. The dark purple line and points overlay the actual values for those 14 days, so you can see how often the actuals fall inside the intervals and how close the point forecast is to the truth. If most actual points lie within the 95\% band, the intervals are well calibrated; if the point forecast tracks the actuals closely, the model has good accuracy.

\begin{figure}[H]
  \centering
  \textbf{[Placeholder: insert ARIMA 14-day forecast with intervals from \texttt{images/arima\_intervals.png}]}
...
\section{Full-Year Time Series Plot (R Code)}\label{appendix:fullyear_ts_plot}
...
\section{14-Day Comparison Plot (R Code)}\label{appendix:comparison_plot}
...
\section{ARIMA 14-Day Forecast with Intervals (R Code)}\label{appendix:arima_intervals_plot}
...
\section{ARIMA Modeling Code}\label{appendix:time_series_modeling_code}
