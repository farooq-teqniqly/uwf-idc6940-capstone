...
The series is split chronologically: the last 14 days are held out as test, the preceding 30 days as validation, and the remainder as training. Both ARIMA and LSTM produce a 14-day forecast aligned to the same test window for a fair comparison. Model evaluation uses sMAPE (symmetric mean absolute percentage error) and MASE (mean absolute scaled error) \cite{hyndman2006,prabhat2024}. These metrics are scale-independent and commonly used in forecasting comparisons; MASE is scaled by the mean absolute error of a naive one-step forecast on the training data \cite{hyndman2006}. Step-by-step implementation details are given in Section~\ref{sec:implementation}.
...
Table~\ref{tab:compare} summarizes sMAPE (\%) and MASE for both methods over the same 14-day period. Figure~\ref{fig:comparison_plot} shows the 14-day actual and forecast series for both models. Plot code is shown in Listing~\ref{lst:comparison_plot}.
