...
Limitations of the methods include: (i)~\textbf{Small sample size}---366 daily observations is on the small side for deep learning. The LSTM has limited training sequences after the chronological split, which may favor the simpler ARIMA model. (ii)~\textbf{Univariate models}---both methods use only the daily average trip duration. (iii)~\textbf{Alignment of evaluation period}---for a fair comparison, ARIMA and LSTM must forecast the same 14-day window. (iv)~\textbf{Library code changes}---if the sMAPE and MASE library code changes, the calculated metrics could change. (v)~\textbf{LSTM reproducibility}---unlike ARIMA, the LSTM forecast (and thus sMAPE and MASE) can vary from run to run because of randomness in weight initialization, the validation split (\texttt{sample()}), and dropout during training. The pipeline fixes the random seed (R \texttt{set.seed(42L)}, TensorFlow \texttt{tf\$random\$set\_seed(42L)}, and NumPy \texttt{np.random.seed(42)}), but runs still produced different metrics in our environment. Therefore the reported LSTM sMAPE and MASE in the comparison table are the \textbf{average of 5 runs} to give a stable, interpretable comparison with ARIMA.
