...
This problem is significant in time-series and data science because it addresses the practical question of short-term forecasting for urban mobility while comparing a classical statistical approach (ARIMA) with a neural sequence model (LSTM) on the same univariate series. The target variable is the mean trip duration per day, aggregated from high-volume for-hire vehicle (FHV) trip records \cite{nyctlc2024}. The inputs are a univariate time series of 366 (2024 was a leap year) daily observations. The outputs are (1) a 14-day point forecast from each method, and (2) a comparison over the same horizon using sMAPE and MASE \cite{hyndman2006,prabhat2024}.

\subsection{Context and Background}

Univariate time-series forecasting is a core problem in statistics and data science. Classical approaches such as ARIMA rely on stationarity, autocorrelation structure, and information criteria (AIC, BIC) for model selection \cite{forecast_auto_arima,hyndman2006}. Deep learning models such as long short-term memory (LSTM) networks can capture nonlinear temporal dependencies and are widely used for sequence prediction \cite{keras_r,keras_timeseries}. Comparative studies of traditional and machine-learning forecasting methods inform the choice of approach for a given problem and data size \cite{prabhat2024}. The data used here are derived from the New York City Taxi and Limousine Commission (TLC) High-Volume FHV trip record data \cite{nyctlc2024}, aggregated to daily average Uber trip duration for 2024.

\subsection{Objectives and Goals}

\begin{itemize}
\item Produce a 14-day point forecast of daily average Uber trip duration using an ARIMA model.
\item Produce a 14-day forecast using an LSTM model on the same univariate series.
...
The primary data source used in the capstone is the CSV file produced by the pipeline above. The dataset is loaded in R via \texttt{read.csv()} (Listing~\ref{lst:load_time_series}). The series has \textbf{366 rows} (one per day in 2024) and \textbf{2 columns}: \texttt{pickup\_date} (Date) and \texttt{avg\_duration\_min} (numeric). There are \textbf{no missing values}. The CSV is located in \texttt{data/time\_series.csv} or \texttt{data/finalproject.csv}. The same file is used by both ARIMA (\texttt{code/arima\_forecast.qmd}) and LSTM (\texttt{code/lstm\_forecast.qmd}). Table~\ref{tab:csv_preview} shows the first five rows of the loaded data (with \texttt{weekday} derived for display).
...
Both methods are evaluated on the same 14-day horizon and the same test period. Metrics are sMAPE and MASE \cite{hyndman2006,prabhat2024}. Results are reported in a single table (ARIMA vs LSTM: sMAPE, MASE). ARIMA and LSTM are appropriate here because the problem is a univariate time series with a moderate number of observations; comparing a classical statistical model with a neural sequence model addresses the question of which approach performs better for this forecasting task.
